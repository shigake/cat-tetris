export class Score {
 ...
 setTimeLevel(level) {
 this._timeLevel = Math.min(15, Math.max(1, level));
 }

 getEffectiveLevel() {
 const lineLevel = this._fixedLevel || this.level;
 return Math.max(lineLevel, this._timeLevel || 1);
 }
 ...
 score._timeLevel = this._timeLevel;
 return score;
 }
