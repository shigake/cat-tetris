...
import { GameEventEmitter, gameEvents, GAME_EVENTS } from '../../patterns/Observer.js';
...
export class GameService extends IGameService {
 ...
 this.events.emit(GAME_EVENTS.GAME_INITIALIZED);
 this._markDirty();
 }
 ...
 this.events.emit(GAME_EVENTS.PIECE_MOVED, { direction, piece: result });
 ...
 this._checkUngrounded();
 this.events.emit(GAME_EVENTS.PIECE_ROTATED, { piece: result });
 }
 }
 ...
 if (this.backToBack) {
 this.events.emit(GAME_EVENTS.BACK_TO_BACK);
 }
 this.backToBack = true;

 if (isTSpin) {
 this.score.addTSpin();
 this.events.emit(GAME_EVENTS.T_SPIN, { linesCleared });
 }
 } else {
 this.backToBack = false;
 }

 this.events.emit(GAME_EVENTS.LINE_CLEARED, { linesCleared });
 this.events.emit(GAME_EVENTS.SCORE_UPDATED, {
 ...
 this.events.emit(GAME_EVENTS.PIECE_PLACED, {
 ...
 attack += comboTable[Math.min(combo, comboTable.length - 1)] || 5;
 }
 ...
 const spawnPos = temp.type === 'I' ? { x: 3, y: -2 } : { x: 3, y: -1 };
 ...
 this.canHold = false;
 this._markDirty();
 this.events.emit(GAME_EVENTS.PIECE_HELD, { heldPiece: this.heldPiece });
 }
 ...
 this.events.emit(GAME_EVENTS.HARD_DROP, { dropDistance: result.dropDistance });
 this.events.emit(GAME_EVENTS.PIECE_MOVED, { direction: 'hardDrop', piece: droppedPiece });
 this.placePiece();
 }
 ...
 this.isPaused = true;
 this._markDirty();
 this.events.emit(GAME_EVENTS.GAME_PAUSED);
 }
 ...
 this.isPlaying = true;
 this._markDirty();
 this.events.emit(GAME_EVENTS.GAME_RESUMED);
 }
 ...
 bufferRows: this.board._buffer.slice(-2).map(row => [...row]),
