...
export default function MainMenu({
 ...
 >
 <img src={`${import.meta.env.BASE_URL}cats/btn-1.png`} alt="" className="w-10 h-10 rounded-full border-2 border-white/30 shadow-md" />
 <div>
 {t('menu.continue')}
 ...
 >
 <img src={`${import.meta.env.BASE_URL}cats/btn-1.png`} alt="" className="w-11 h-11 rounded-full border-2 border-white/30 shadow-md" />
 {t('menu.play')}
 </button>
 )}
 </motion.div>
 ...
 <MenuCard icon={<SwordsIcon size={22} />} catImg={`${import.meta.env.BASE_URL}cats/btn-2.png`} label={t('menu.vsAI')} sub={t('menu.vsAISub')} onClick={() => play(onShowMultiplayer)} selected={isSelected('vsAI')} />
 <MenuCard icon={<BookIcon size={22} />} catImg={`${import.meta.env.BASE_URL}cats/btn-3.png`} label={t('menu.tutorial')} sub={t('menu.tutorialSub')} onClick={() => play(onShowTutorialHub)} selected={isSelected('tutorial')} />
 <MenuCard icon={<PaletteIcon size={22} />} catImg={`${import.meta.env.BASE_URL}cats/btn-4.png`} label={t('menu.creator')} sub={t('menu.creatorSub')} onClick={() => play(onShowCreatorMode)} selected={isSelected('creator')} />
 <MenuCard icon={<BrainIcon size={22} />} catImg={`${import.meta.env.BASE_URL}cats/btn-5.png`} label={t('menu.aiExpert')} sub={t('menu.aiExpertSub')} onClick={() => play(onShowAIShowcase)} selected={isSelected('aiExpert')} />
 </motion.div>

 <motion.div {...fadeUp(0.25)} className="w-full grid grid-cols-4 gap-1 mb-4">
 <QuickBtn icon={<ShopBagIcon size={20} />} catImg={`${import.meta.env.BASE_URL}cats/btn-6.png`} label={t('menu.shop')} onClick={() => play(onShowShop)} selected={isSelected('shop')} />
 <QuickBtn icon={<ClipboardIcon size={20} />} catImg={`${import.meta.env.BASE_URL}cats/btn-6.png`} label={t('menu.missions')} onClick={() => play(onShowMissions)} selected={isSelected('missions')} />
 <QuickBtn icon={<TrophyIcon size={20} />} catImg={`${import.meta.env.BASE_URL}cats/btn-6.png`} label={t('menu.achievements')} onClick={() => play(onShowAchievements)} selected={isSelected('achievements')} />
 <QuickBtn icon={<GearIcon size={20} />} catImg={`${import.meta.env.BASE_URL}cats/btn-6.png`} label={t('menu.settings')} onClick={() => play(onShowSettings)} selected={isSelected('settings')} />
 </motion.div>
...
function MenuCard({ icon, catImg, label, sub, onClick, selected }) {
 ...
 >
 {catImg && <img src={catImg} alt="" className="w-14 h-14 rounded-xl mb-1.5 object-cover drop-shadow-lg" />}
...
function QuickBtn({ icon, catImg, label, onClick, selected }) {
 ...
 <button
 onClick={onClick}
 className={`relative flex flex-col items-center gap-1 px-2 py-2 rounded-lg hover:bg-white/[0.07] active:scale-95 transition-all duration-150 overflow-hidden ${selected ? 'ring-2 ring-yellow-400 bg-white/[0.1] scale-105' : ''}`}
 >
 {catImg && <img src={catImg} alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.06] pointer-events-none rounded-lg" />}
